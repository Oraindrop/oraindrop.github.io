...
- upgardeLevels() 리팩토링
- ![upgardeLevel](https://github.com/Oraindrop/oraindrop.github.io/blob/master/assets/_img/upgardeLevel.png?raw=true)
    - 1 : 현재 레벨 파악
    - 2 : 업그레이드 조건 판단
    - 3 : 레벨 업그레이드
    - 4 : DB 저장
...
- 추상적으로, upgardeLevels() 에는 loop 을 돌며 업그레이드가 가능하면 업그레이드 한다. 라는 작업 흐름만 명시
```
List<User> users = userDao.getAll();
...
- 트랜잭션 동기화 방식을 적용한 upgardeLevels()
```
...
