...
- 프록시의 특징은 타깃과 같은 인터페이스를 구현했따는 것과, 프록시가 타깃을 제어할 수 있는 위치에 있다는 것.
...
- 타깃과 동일한 인터페이스를 구현하고 클라이언트와 타깃 사이에 존재하면서 기능으 ㅣ부가 또는 접근 제어를 담당하는 오브젝트는 모두 프록시
